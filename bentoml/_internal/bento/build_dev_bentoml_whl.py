...
def build_bentoml_whl_to_target_if_requested(target_path, bundle_local_build):
    ...
    """
    if not os.environ.get("BENTOML_BUNDLE_LOCAL_BUILD", None) and not bundle_local_build:
        return
    ...
    logger.info(
        "BentoML is installed in `editable` model, building BentoML distribution with local BentoML code base. "
